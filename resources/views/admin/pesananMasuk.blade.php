    ...
    <script>
        $(document).ready(function(){
            ...
            table = $('#table_pesanan').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{route('pengelola.pesanan.masuk.ajax')}}",
                    "type": "POST",
                    "data": function (d) {
                        d._token = "{{csrf_token()}}"
                    },
        ...
        function confirmSwal(id_pesanan){
            Swal.fire({
            title: 'Antar pesanan?',
            text: "Pastikan anda sudah mencetak struk dan menempelkannya pada makanan, pesanan akan diteruskan kepada waiters",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Ya Antar Pesanan'
            }).then((result) => {
        ...
        function detailPesanan(id){
            $.get("{{url('pengelola/pesanan/detail')}}"+"/"+id, function(data, status){
                ...
                switch(data.status){
                    ...
                    break;
                    // case 'Diantar':
                    //     $('#change_status_order').val('Selesai')
                    //     $('#btn-container').html(`<button type="button" class="btn btn-primary" onclick="confirmSwal(${data.id})">Antar Pesanan</button>`)
                    // break;
                }
